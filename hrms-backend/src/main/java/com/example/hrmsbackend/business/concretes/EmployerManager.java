...
import com.example.hrmsbackend.dataAccess.abstracts.EmployerDao;
import com.example.hrmsbackend.dataAccess.abstracts.EmployerUpdateDao;
...
public class EmployerManager extends BaseManager<EmployerDao, Employer, Integer> implements EmployerService {
	...
	private Result arePasswordMatch(final String password, final String confirmPassword) {
		return password.equals(confirmPassword) ? new SuccessResult()
				: new ErrorResult(com.example.hrmsbackend.core.business.constants.Messages.passwordsNotMatch);
	}
	...
	public DataResult<EmployerUpdate> getLastUpdateByUserId(final int employerId) {
		...
		if (employerUpdates.size() == 0)
			return new ErrorDataResult<>(com.example.hrmsbackend.core.business.constants.Messages
	...
	public Result isNotCorporateEmailExist(final String corporateEmail) {
		return employerDao.findByCorporateEmail(corporateEmail).isEmpty() ? new SuccessResult()
				: new ErrorResult(com.example.hrmsbackend.core.business.constants.Messages
	...
	public Result register(final EmployerForRegisterDto employerForRegister) {
		...
		return new SuccessResult(
				com.example.hrmsbackend.core.business.constants.Messages.complete("Employer registration"));
	}
	...
	public Result updateByUser(final EmployerForUpdateDto employerForUpdateDto, MultipartFile companyImage) {
		...
		if (employer.isEmpty())
			return new ErrorResult(
					com.example.hrmsbackend.core.business.constants.Messages.notFound("Employer"));
		...
		return new SuccessResult(
				com.example.hrmsbackend.core.business.constants.Messages.awaitingApproval("Employer update"));
	}
	...
	public Result verifyUpdate(final int employerUpdateId) {
		...
		if (employerUpdate.isEmpty())
			return new ErrorResult(
					com.example.hrmsbackend.core.business.constants.Messages.notFound("Employer update"));
		...
		return new SuccessResult(
				com.example.hrmsbackend.core.business.constants.Messages.verified("Employer update"));
	}
	...
	public Result denyUpdate(final int employerUpdateId) {
		...
		return new SuccessResult(
				com.example.hrmsbackend.core.business.constants.Messages.denied("Employer update"));
	}
}
