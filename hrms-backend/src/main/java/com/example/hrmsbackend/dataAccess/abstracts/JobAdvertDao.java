...
public interface JobAdvertDao extends JpaRepository<JobAdvert, Integer> {
	...
	@Query("SELECT new com.example.hrmsbackend.entities.dtos.JobAdvertForListDto(j.id,e.companyName,e.companyImageUrl,c.name,p.title,j.numberOfOpenPositions,wt.name,j.createdAt,j.applicationDeadline,j.minSalary,j.maxSalary) FROM JobAdvert j JOIN j.employer e JOIN j.jobPosition p JOIN j.city c JOIN j.workingTime wt WHERE j.isActive=:isActive AND e.companyName=:companyName")
	...
	Page<JobAdvertForListDto> findAllByIsActiveAndCityAndWorkingTimeForList(@Param("isActive") boolean isActive,
			...
			@Param("workingTimeId") short workingTimeId,
			Pageable pageable);
}